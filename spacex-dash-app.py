...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # For all sites, show total success counts
        fig = px.pie(
            spacex_df,
            values='class',                 # counts of success (class=1)
            names='Launch Site',            # grouped by launch site
...
def update_scatter(selected_site, payload_range):
    ...
    df_filt = spacex_df[(spacex_df['Payload Mass (kg)'] >= low) &
                        (spacex_df['Payload Mass (kg)'] <= high)]

    if selected_site != 'ALL':
        df_filt = df_filt[df_filt['Launch Site'] == selected_site]
